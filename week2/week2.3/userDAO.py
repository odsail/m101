...
class UserDAO:
    ...
    def validate_login(self, username, password):
        ...
        try:
            ...
            #print(password)
            #user = self.users.findOne({"_id":username, "password":password})
            #user = self.users.find_one( { "_id": "scott" } ).limit(1)
            user = self.users.find_one({'_id': username, "password":password})
            print(user)
            #print(self.users.findOne({"_id" : username, "password":password }))
            #print("Updated: This space intentionally left blank.")
        except:
